  ...
  for ss in ${sesDates}; do
    ...
    if [ "$nT1w" -gt "$runmax" ]; then
      ...
      for fname in ${T1w}; do
        ...
        imagefolder="anat"
        imagefile="T1w"
        newname="sub-${s}_ses-${ssnum}_run-1_${imagefile}.${ext}"
        
        mv ${fname} ${newname}
        3drefit -denote ${newname} 
        newname="sub-${s}_ses-${ssnum}_run-1_${imagefile}.json"
...
data_in_orig=/projects/NHP_processed/developmental_in
...
for s in $(seq -f "%03g" 1 3); do
  ...
  for ss in ${sesDates}; do
    ...
    rm -rf ${data_in}/sub-${s}/ses-${ssnum}/func
    rm -rf ${data_in}/sub-${s}/ses-${ssnum}/fmap
    ...
    for fname in ${rest}; do
      ...
      if [ "$vols" == "16" ]; then 
        ...
        imagefile="bold"
        newname="sub-${s}_ses-${ssnum}_${task}_run-${nrest}_${scan}.${ext}"
        ln -s ${data_in_orig}/sub-${s}/ses-${ssnum}/func/${fname} ${data_in}/sub-${s}/ses-${ssnum}/func/${newname}
        #mv ${fname} ${newname}
        #3drefit -denote ${newname} 

        newname="sub-${s}_ses-${ssnum}_${task}_run-${nrest}_${scan}.json"

        ln -s ${data_in_orig}/sub-${s}/ses-${ssnum}/func/${jname} ${data_in}/sub-${s}/ses-${ssnum}/func/${newname}
    ...
    for fname in ${rest}; do
      ...
      if [ "$vols" == "16" ]; then
        ...
        newname="sub-${s}_ses-${ssnum}_${task}_acq-${nrev}_dir-AP_epi.nii.gz"
        ln -s ${data_in_orig}/sub-${s}/ses-${ssnum}/fmap/${fname} ${data_in}/sub-${s}/ses-${ssnum}/fmap/${newname}
        #mv ${fname} ../fmap/${newname}
        #3drefit -denote ../fmap/${newname} 

        newname="sub-${s}_ses-${ssnum}_${task}_acq-${nrev}_dir-AP_epi.json"
        ln -s ${data_in_orig}/sub-${s}/ses-${ssnum}/fmap/${jname} ${data_in}/sub-${s}/ses-${ssnum}/fmap/${newname}
  ...
  for ss in ${sesDates}; do
    ...
    if [ "$nT1w" -gt "$runmax" ]; then
      ...
      for fname in ${T1w}; do
        ...
        #cp ${fname} ${data_in}/sub-${s}/ses-${ssnum}/anat/${newname}
        newname="sub-${s}_ses-${ssnum}_${run}_${scan}.json"
        fileLength=$(wc -l ${newname} | cut -d' ' -f1); 
        head -$((fileLength-1)) ${newname} > temp.json

        #sed -i -e "s|${end}|${end},|g" temp.json 
        cat temp.json ${scripts_path}/add2json.json > final.json
        mv final.json ${newname}
        rm temp.json
        #cp ${jname} ${data_in}/sub-${s}/ses-${ssnum}/anat/${newname}
      done
    else
