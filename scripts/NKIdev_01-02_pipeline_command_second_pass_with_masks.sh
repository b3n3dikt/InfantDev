...
echo "docker run -i --rm -v ${INPUT}:/input -v ${OUTPUT}:/output -v ${TEMPLATES}:/atlases -v ${PARCELLATIONS}:/opt/dcan-tools/dcan_bold_proc/templates/parcellations -v ${fMRIVolume}:/opt/pipeline/fMRIVolume -v ${MASK}:/masks -v /home/bramirez/pipelines/freesurfer/license.txt:/opt/freesurfer/license.txt dcanumn/nhp-abcd-bids-pipeline /input /output --participant-label ${subname} --session-id [ ${sesh} ] --ncpus ${cores} --t1-brain-mask /masks/sub-${subname}/ses-${sesh}/T1w_average_pre_mask.nii.gz --multi-template-dir /atlases/${JLC} --hyper-normalization-method ROI_IPS --make-white-from-norm-t1 --single-pass-pial --max-cortical-thickness 30 --freesurfer-license=/opt/freesurfer/license.txt " >> ${OUTPUT}/proc_times/${subname}_${sesh}.txt

#sesh=20201015
#cores=1
#echo "DELETE THIS WHEN YOU HAVE ADDED THE PATHS TO THE NEWLY MADE ASEG FILES"
#docker run -i --rm -v ${INPUT}:/input -v ${OUTPUT}:/output -v ${TEMPLATES}:/atlases -v ${PARCELLATIONS}:/opt/dcan-tools/dcan_bold_proc/templates/parcellations -v ${fMRIVolume}:/opt/pipeline/fMRIVolume -v ${PreFreeSurfer}:/opt/pipeline/PreFreeSurfer -v ${MASK}:/masks -v /home/bramirez/pipelines/freesurfer/license.txt:/opt/freesurfer/license.txt dcanumn/nhp-abcd-bids-pipeline /input /output --participant-label ${subname} --session-id [ ${sesh} ] --ncpus ${cores} --study-template ${sTEMPLATE_head} ${sTEMPLATE_brain}--t1-brain-mask /masks/sub-${subname}/ses-${sesh}/T1w_average_pre_mask.nii.gz --multi-template-dir /atlases/${JLC} --hyper-normalization-method ROI_IPS --make-white-from-norm-t1 --single-pass-pial --max-cortical-thickness 30 --freesurfer-license=/opt/freesurfer/license.txt --stage PreFreeSurfer

docker run -i --rm -v ${INPUT}:/input -v ${OUTPUT}:/output -v ${TEMPLATES}:/atlases -v ${PARCELLATIONS}:/opt/dcan-tools/dcan_bold_proc/templates/parcellations -v ${fMRIVolume}:/opt/pipeline/fMRIVolume -v ${PreFreeSurfer}:/opt/pipeline/PreFreeSurfer -v ${MASK}:/masks -v /home/bramirez/pipelines/freesurfer/license.txt:/opt/freesurfer/license.txt dcanumn/nhp-abcd-bids-pipeline /input /output --participant-label ${subname} --session-id [ ${sesh} ] --ncpus ${cores} --t1-brain-mask /masks/sub-${subname}/ses-${sesh}/T1w_average_pre_mask.nii.gz --multi-template-dir /atlases/${JLC} --hyper-normalization-method ROI_IPS --make-white-from-norm-t1 --single-pass-pial --max-cortical-thickness 30 --freesurfer-license=/opt/freesurfer/license.txt
#--print-commands-only
